...
contract WireTestParent {
    ...
    function inherited(uint256[2] memory) public {
        emit Done();
    }

    //no constructor

    function inheritedReturn() public pure returns (uint256) {
        return 1;
    }
}
...
abstract contract WireTestAbstract {
    ...
    function overriddenReturn() public pure virtual returns (uint256);
}
...
contract WireTest is WireTestParent, WireTestAbstract {
    ...
    event MoreStuff(WireTest, uint256[] data);

    function moreStuff(WireTest notThis, uint256[] memory bunchOfInts) public {
    ...
    event HasIndices(uint256, uint256 indexed, string, string indexed, uint256);
    ...
    event AmbiguousEvent(uint8[] indexed, uint256[5]);

    function ambiguityTest() public {
        uint8[] memory short = new uint8[](3);
        uint256[5] memory long;
        long[0] = 32;
        long[1] = 3;
        long[2] = short[0] = 17;
        long[3] = short[1] = 18;
    ...
    event AnonUints(uint256 indexed, uint256 indexed, uint256 indexed, uint256 indexed) anonymous;
    event NonAnon(uint256 indexed, uint256 indexed, uint256 indexed);
    ...
    function overriddenReturn() public pure override returns (uint256) {
        return 2;
    }

    receive() external payable {}

    function boom() public returns (uint256) {
        selfdestruct(payable(address((this))));
    }

    event SemiAmbiguousEvent(uint256 indexed, uint256);
    ...
    function extrasTestNone(address test) public returns (bool, bytes memory) {
        return test.delegatecall(abi.encodeWithSignature("run()"));
    }

    error UnambiguousError(int256, int256);
...
library WireTestLibrary {
    ...
    error b27072(uint256); //ambiguous with h9316(bytes32)
...
contract WireTestRedHerring {
    event SemiAmbiguousEvent(uint256, uint256 indexed);
